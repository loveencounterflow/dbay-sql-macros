(function() {
  ...
  * All codepoint collections...
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_any_name = function(prefix = '@', mode = 'practical') {
    return RegExp(`${prefix}${this.rx.chrs[mode].allowed.head.source}${this.rx.chrs[mode].allowed.tail.source}*`, "sgu");
  };

  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_bare_name = function(prefix = '@', mode = 'practical') {
    return RegExp(`${prefix}${this.rx.chrs[mode].allowed.head.source}${this.rx.chrs[mode].allowed.tail.source}*(?!${this.rx.chrs[mode].forbidden.paren.source})`, "sgu");
  };

  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_paren_name = function(prefix = '@', mode = 'practical') {
    return RegExp(`${prefix}${this.rx.chrs[mode].allowed.head.source}${this.rx.chrs[mode].allowed.tail.source}*(?=[(])`, "sgu");
  };

  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_start_paren_name = function(prefix = '@', mode = 'practical') {
    return RegExp(`^${prefix}${this.rx.chrs[mode].allowed.head.source}${this.rx.chrs[mode].allowed.tail.source}*(?=[(])`, "u");
  };

  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_parameter = function(mode = 'practical', vanisher = '|', parameter) {
    return RegExp(`(?:(?<!\\\\)${GUY.str.escape_for_regex(parameter)}${GUY.str.escape_for_regex(vanisher)})|(?:(?<!\\\\)${GUY.str.escape_for_regex(parameter)}(?!${this.rx.chrs[mode].forbidden.paren.source}))`, "gu");
  };
