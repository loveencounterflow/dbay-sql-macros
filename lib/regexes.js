(function() {
  ...
  * All codepoint collections...
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_any_name = function(cfg) {
    return RegExp(`(?<!${cfg._escape_esc})${cfg._prefix_esc}${this.rx.chrs[cfg.mode].allowed.head.source}${this.rx.chrs[cfg.mode].allowed.tail.source}*`, "sgu");
  };

  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_bare_name = function(cfg) {
    return RegExp(`(?<!${cfg._escape_esc})${cfg._prefix_esc}${this.rx.chrs[cfg.mode].allowed.head.source}${this.rx.chrs[cfg.mode].allowed.tail.source}*(?!${this.rx.chrs[cfg.mode].forbidden.paren.source})`, "sgu");
  };

  //-----------------------------------------------------------------------------------------------------------
  this.get_rx_for_paren_name = function(cfg) {
    return RegExp(`(?<!${cfg._escape_esc})${cfg._prefix_esc}${this.rx.chrs[cfg.mode].allowed.head.source}${this.rx.chrs[cfg.mode].allowed.tail.source}*(?=[(])`, "sgu");
  };
  ...
  this.get_rx_for_parameter = function(cfg, parameter) {
    return RegExp(`(?<!${cfg._escape_esc})${GUY.str.escape_for_regex(parameter)}(?:(?!${this.rx.chrs[cfg.mode].forbidden.paren.source}))`, "gu");
  };
