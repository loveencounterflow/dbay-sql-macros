    ...
    _declare(cfg) {
      ...
      cfg.parameter_res = (function() {
        ...
        for (i = 0, len = ref1.length; i < len; i++) {
          p = ref1[i];
          results.push(rx.get_rx_for_parameter(this.cfg, p));
        }
        return results;
      }).call(this);
...
// help '^56-2^', ( rpr tail ), '->', GUY.trm.reverse GUY.trm.steel values
      ...
      R = R.replaceAll(this.cfg._escaped_escape_re, (..._) => {
        var esc, ref1;
        ref1 = _, [..._] = ref1, [{esc}] = splice.call(_, -1);
        if (esc.length % 2 === 1) {
          return this.cfg.escape.repeat((esc.length - 1) / 2);
        } else {
          return this.cfg.escape.repeat(esc.length / 2);
        }
      });
      return R;
    }
