(function() {
  'use strict';
  var DBay_sqlm_TOBESPECIFIED_error, DBay_sqlm_arity_error, DBay_sqlm_circular_references_error, DBay_sqlm_duplicate_parameters_error, DBay_sqlm_error, DBay_sqlm_internal_error, DBay_sqlm_recursion_level_error, DBay_sqlm_unknown_macro_error, DBay_sqlm_unknown_parameters_error, DBay_sqlx, GUY, alert, debug, echo, equals, help, info, inspect, log, plain, praise, rpr, rx, sql_lexer, urge, warn, whisper,
    indexOf = [].indexOf;
  ...
  DBay_sqlm_circular_references_error = class DBay_sqlm_circular_references_error extends DBay_sqlm_error {
    constructor(ref, names) {
      var n;
      super(ref, `circular references involving macros ${((function() {
        var results;
        results = [];
        for (n of names) {
          results.push(rpr(n));
...
// help '^56-2^', ( rpr tail ), '->', GUY.trm.reverse GUY.trm.steel values
        for (value_idx = i = 0, len = values.length; i < len; value_idx = ++i) {
          ...
          if ((value.match(pnre)) != null) {
            value = this._resolve(value, level + 1, seen);
          }
        ...
        if ((body.match(pnre)) != null) {
          //.....................................................................................................
          body = this._resolve(body, level + 1, seen);
        }
        R.push(body);
        position += stop_idx;
        count++;
