(function() {
  'use strict';
  var DBay_sqlm_TOBESPECIFIED_error, DBay_sqlm_arity_error, DBay_sqlm_error, DBay_sqlm_internal_error, DBay_sqlm_unknown_macro_error, DBay_sqlx, GUY, alert, debug, echo, equals, help, info, inspect, log, plain, praise, rpr, rx, sql_lexer, urge, warn, whisper;
...
values: ${rpr(values)}`);
  ...
  DBay_sqlx = class DBay_sqlx { // extends ( require H.dbay_path ).DBay
    ...
    _resolve(sqlx, level) {
      var R, body, call_arity, count, declaration, i, len, m, match, name, pnre, position, source, stop_idx, tail, value, value_idx, values;
      this.types.validate.nonempty.text(sqlx);
      R = [];
      position = 0;
...
// help '^56-2^', ( rpr tail ), '->', GUY.trm.reverse GUY.trm.steel values
        for (value_idx = i = 0, len = values.length; i < len; value_idx = ++i) {
          ...
          debug('^35345^', (function() {
            var ref1, results;
            ref1 = body.matchAll(declaration.parameter_res[value_idx]);
            results = [];
            for (m of ref1) {
              results.push(m);
            }
