(function() {
  'use strict';
  var DBay_sqlm_TOBESPECIFIED_error, DBay_sqlm_error, DBay_sqlm_internal_error, DBay_sqlm_unknown_macro_error, DBay_sqlx, GUY, alert, debug, echo, equals, help, info, inspect, log, plain, praise, rpr, sql_lexer, urge, warn, whisper;
  ...
  DBay_sqlx = class DBay_sqlx { // extends ( require H.dbay_path ).DBay
    ...
    resolve(sqlx) {
      ...
      var R, body, call_arity, declaration, i, len, match, name, pnre, position, stop_idx, tail, value, value_idx, values;
      // whisper '---------------------------------'
      // help '^56-1^', rpr sqlx
      this.types.validate.nonempty.text(sqlx);
      R = [];
