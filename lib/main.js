(function() {
  ...
  DBay_sqlx = class DBay_sqlx { // extends ( require H.dbay_path ).DBay
    ...
    _declare(cfg) {
      ...
      cfg.parameter_res = (function() {
        ...
        for (i = 0, len = ref1.length; i < len; i++) {
          p = ref1[i];
          results.push(GUY.str.escape_for_regex(p));
        }
        return results;
      })();
    ...
    resolve(sqlx) {
      ...
      var R, body, call_arity, declaration, i, last_idx, len, match, name, parameter, parameter_idx, position, ref1, ref2, sql_before, stop_idx, tail, values;
      this.types.validate.nonempty.text(sqlx);
      sql_before = sqlx;
      position = 0;
      R = [];
      this.cfg._global_name_re.lastIndex = 0;
      ...
      for (match of ref1) {
        ...
        for (parameter_idx = i = 0, len = ref2.length; i < len; parameter_idx = ++i) {
          parameter = ref2[parameter_idx];
          body = body.replace(RegExp(`${parameter}\\b`, "gu"), values[parameter_idx]);
        }
        //.....................................................................................................
        R.push(body);
