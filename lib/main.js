(function() {
  'use strict';
  var DBay_sqlm_TOBESPECIFIED_error, DBay_sqlm_arity_error, DBay_sqlm_error, DBay_sqlm_internal_error, DBay_sqlm_unknown_macro_error, DBay_sqlm_unknown_parameters_error, DBay_sqlx, GUY, alert, debug, echo, equals, help, info, inspect, log, plain, praise, rpr, rx, sql_lexer, urge, warn, whisper,
    indexOf = [].indexOf;
...
values: ${rpr(values)}`);
  ...
  DBay_sqlx = class DBay_sqlx { // extends ( require H.dbay_path ).DBay
    ...
    _validate_parameters(declared_parameters, body) {
      var p, unknown_parameters, used_parameters;
      used_parameters = body.match(this.cfg._bare_name_re);
      unknown_parameters = (function() {
        var i, len, results;
        results = [];
        for (i = 0, len = used_parameters.length; i < len; i++) {
          p = used_parameters[i];
    ...
    resolve(sqlx) {
      return this._resolve(sqlx, 0);
    }

    _resolve(sqlx, level) {
      /* NOTE using a function to avoid accidental replacement semantics */
      var R, body, call_arity, count, declaration, i, len, match, name, pnre, position, source, stop_idx, tail, value, value_idx, values;
      this.types.validate.nonempty.text(sqlx);
      R = [];
      position = 0;
...
// help '^56-2^', ( rpr tail ), '->', GUY.trm.reverse GUY.trm.steel values
        for (value_idx = i = 0, len = values.length; i < len; value_idx = ++i) {
          ...
          if ((value.match(pnre)) != null) {
            value = this.resolve(value, level + 1);
          }
        ...
        if ((body.match(pnre)) != null) {
          //.....................................................................................................
          body = this.resolve(body, level + 1);
        }
        R.push(body);
        position += stop_idx;
        count++;
